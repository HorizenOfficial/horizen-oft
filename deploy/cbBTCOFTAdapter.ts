...
const contractName = 'cbBTCOFTAdapter'
...
const deploy: DeployFunction = async (hre) => {
    ...
    console.log(`Adapting token address: ${hre.network.config.oftAdapter.tokenAddress}`)
    
    const { address } = await deploy(contractName, {
        from: deployer,
        args: [
            hre.network.config.oftAdapter.tokenAddress, // token address
            endpointV2Deployment.address, //"0x6EDCE65403992e310A62460808c4b910D972f10f", // LayerZero's EndpointV2 address
            deployer, // owner
        ],
        log: true,
